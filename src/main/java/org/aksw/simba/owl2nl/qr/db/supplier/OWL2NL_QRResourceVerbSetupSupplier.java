...
public class OWL2NL_QRResourceVerbSetupSupplier extends OWL2NL_QRSetupSupplier {
    private static final String STARTED_EXPERIMENTS_QUERY = "SELECT resourceId FROM ResourceExperiments WHERE resourceId NOT INT (SELECT resourceId FROM ResourceExperiments WHERE userId=?);";
    ...
    private static final String RESOURCE_QUERY = "SELECT verbalization FROM Resources WHERE R.id=?;";
